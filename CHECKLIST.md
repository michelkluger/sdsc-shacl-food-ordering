...
|---|---|---|
| **Meilisearch**, in Docker | Search documents are projected from the loaded catalogue including option labels, so a new dish is searchable with no indexing rule written for it. Surfaced in the UI as the dish rail's search box and facet filters, which are built from the facets the response returns | `search/indexer.py`, `frontend/src/DishBrowser.tsx`; `GET /api/search?q=cashew` → poke-bowl |
...
| **Validation** — meaningful, with clear structured errors | Every constraint class exercised, incl. three `sh:sparql` cross-field rules. Errors carry a JSON pointer to the offending array *element*, the constraint component, the value, and the shape's own `sh:message`. The submission boundary is policed separately from the shapes: JSON-LD keywords are refused before lifting, because `sh:closed` polices predicates and a keyword never becomes one |
| **Approach to testing** — evidence valid and invalid submissions were tested | 472 backend + 85 frontend tests. 22 order fixtures across 3 dishes, each invalid one declaring the pointer and constraint it must trigger, exercised in all 5 languages. Beyond well-formed submissions, `tests/contract/test_payload_boundary.py` asserts that a client cannot change the terms it is judged on. Integration tests run against real Meilisearch in CI |
...
| Tests | 378 backend (unit · contract · API · integration), 86 frontend |
| Coverage | Gate at 85% |
